...
4. Physical-device latency profiling on iPhone 15 Pro Max (A17 Pro) demonstrating that the 96KB Core ML model meets latency thresholds with wide margin (p50 = 14.0 ms). Energy and memory profiling, the 70% EM quality target, and a production-grade model remain outstanding.
...
**Table 2b: Physical-device latency for tiny_multimodal_v0**
...
Both physical-device sessions pass all latency thresholds with wide margin (p50 <= 400 ms: 14.0 ms actual; p95 <= 600 ms: 26.2 ms actual). The physical device is approximately 7x faster than the Simulator in Release build (14 ms vs 98 ms), consistent with A17 Pro Neural Engine acceleration. A 3.6 MB Time Profiler trace was captured during Session 2 for CPU/thread analysis.
...
| tiny_multimodal_v0 | 0.100 | 14.0 | Physical device | 0.5 | Yes |

Both models are trivially Pareto-optimal because only two points exist and they have equal quality. The latency value now reflects physical-device measurement (iPhone 15 Pro Max, A17 Pro) rather than Simulator. The Pareto frontier will become meaningful once models with different quality-latency trade-offs are added.
...
Physical-device latency profiling is now available from 2 sessions on AT-X (iPhone 15 Pro Max, A17 Pro), with p50 = 14.0 ms and p95 = 26.2 ms -- well within thresholds. However, the following conclusions still require additional physical-device profiling:
...
The primary contribution of this work at its current stage is **infrastructure validation with preliminary physical-device latency evidence**. We have demonstrated:
...
4. **Physical-device latency profiling** on iPhone 15 Pro Max (A17 Pro) showing that even a minimal 96KB Core ML model achieves p50 = 14.0 ms inference latency -- 28x below the 400 ms threshold and 7x faster than iOS Simulator. This confirms that the latency constraint is not the binding bottleneck for the current model; quality (EM) is the binding constraint.
